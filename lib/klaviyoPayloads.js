 ...
 * Event-level fields (order_id, items, value, id/name/url, location_id, etc.) → attributes.properties.
...
export function buildKlaviyoEventPayload(ev) {
  ...
  if (ev.lineItem && typeof ev.lineItem === 'object') {
    ...
    properties.id = li.productId ?? li.subscriptionId ?? li.serviceId ?? li.id
    properties.name = li.name
    properties.url = li.url ?? ''
    ...
    if (li.brand) properties.brand = li.brand
  ...
  for (const [key, value] of Object.entries(ev)) {
    ...
    // Never send currency/value_currency (user-visible payload preview expects them removed).
    if (key === 'valueCurrency' || key === 'value' || key === 'currency' || key === 'value_currency') continue
    properties[key] = value
  }
